...
def printerPrintheadCommand():
	...
	if not printer.is_operational() or (printer.is_printing() and (command != "feedrate" or command != "fastbot") ):
...
def printerCommand():
	...
	if "command" in data and "cmd" in data["command"]: #lkj 
		cmd = data["command"]		
		cmd = cmd.replace("cmd", "")
		printer.shellcommand(cmd)		
	elif "command" in data or "commands" in data:
		if "command" in data:
