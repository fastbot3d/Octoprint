...
def index():
	...
	templates["settings"]["entries"] = dict(
		...
		#lkj serial=(gettext("Serial Connection"), dict(template="dialogs/settings/serialconnection.jinja2", _div="settings_serialConnection", custom_bindings=False)),
		...
		#lkj features=(gettext("Features"), dict(template="dialogs/settings/features.jinja2", _div="settings_features", custom_bindings=False)),
	...
	eeprom_buf  = None
	try:
		eeprom_read = open('/sys/bus/i2c/devices/1-0057/eeprom')				
		eeprom_buf = eeprom_read.read(12)
	except:
		print "eeprom_read except"
	finally:
		if eeprom_read is not None:
			eeprom_read.close()	
	board_type = "BBP1S"	 #BBP1, BBP1S
	if eeprom_buf is not None:
		eeprom_buf = eeprom_buf[4:]
		eeprom_buf = eeprom_buf.strip('\x00')
		board_type = eeprom_buf
	print("board_type=%s" % str(board_type))
