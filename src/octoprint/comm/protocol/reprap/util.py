...
class GcodeCommand(object):
	...
	KNOWN_FLOAT_ATTRIBUTES = ("b", "d", "i", "a", "x", "y", "z", "e", "s", "p", "r", "w")
		...
		if self.original is not None:
			...
			for key in GcodeCommand.KNOWN_ATTRIBUTES:
				try:
					...
					value = None
				#print("lkj gccode command key:%s" % key)
				if value is not None:
					if key in GcodeCommand.KNOWN_INT_ATTRIBUTES:
						#attr.append("%s%d" % (key.upper(), value))
						attr.append("%s%d" % (key.upper(), int(value)))
					elif key in GcodeCommand.KNOWN_FLOAT_ATTRIBUTES:
						attr.append("%s%f" % (key.upper(), float(value)))
					else:
...
class PrintingGcodeFileInformation(PrintingFileInformation):
	...
	def __init__(self, filename, offsetCallback, origin=FileDestinations.LOCAL):
