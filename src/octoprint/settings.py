...
default_settings = {
	...
	"terminalFilters": [
		{ "name": "Suppress M105 requests/responses", "regex": "(M105)|(Recv: OK *)|(Recv:    T\d*:)" },
	...
	"scripts": {
		#"gcode": {                        
                #lkj "afterPrintCancelled": "; disable motors\nM84\n\n;disable all heaters\n{% snippet 'disable_hotends' %}\nM140 S0\n\n;disable fan\nM106 S0",
                        #lkj "snippets": {
                        #lkj	"disable_hotends": "{% for tool in range(printer_profile.extruder.count) %}M104 T{{ tool }} S0\n{% endfor %}"
                        #}                			
		#}              
	},
	"devel": {
...
class Settings(object):
	...
	def setStaticIP(self):
		ip = self.get(["fastbotArgs", "staticIPAddress"])
		netMask = self.get(["fastbotArgs", "staticIPNetmask"])
		gateWay = self.get(["fastbotArgs", "staticIPGateWay"])
	...
	def _migrate_printer_parameters(self):
		...
		if "volume" in default_profile:  #lkj
			if "formFactor" in  default_profile["volume"] and "origin" in  default_profile["volume"] and "machineType" in default_profile:
				if "Delta" in default_profile["machineType"]:
					default_profile["volume"]["formFactor"] = "circular"
					default_profile["volume"]["origin"] = "center"	
		if "measure_ext1" not in default_profile:  #lkj
			default_profile["measure_ext1"] = "1"
			default_profile["measure_ext2"] = "2"
			default_profile["measure_ext3"] = "3"				
